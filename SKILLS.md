...
* [cite_start]**100/80/0 Strategy**: Apply **100% coverage** to CORE (domain logic/AI evaluators), **80% coverage** to GLOBAL (application/UI), and **0% coverage** to INFRA (static config/mocks)[cite: 138, 139, 140, 141, 173, 176, 180, 185].
* [cite_start]**Automated Quality Gates (Husky)**: Implement a mandatory `pre-push` hook using Husky to block any code that fails the test suite or falls below the 80% global coverage threshold[cite: 66, 69, 148, 198, 203, 325].
* **Mac-Compatible Setup**: Always ensure the hook is executable by running `chmod +x .husky/pre-push` during the project setup phase.
* [cite_start]**Deterministic Mocking**: All external AI and infrastructure services (OpenAI, PDF Parsers) must be mocked to ensure consistent, cost-effective test results[cite: 331, 332].
* [cite_start]**Traceability Validation**: Every AI-generated suggestion must include textual evidence and be recorded in a `DecisionLog` to ensure system transparency[cite: 20, 154, 274, 338].
* [cite_start]**Code Smell Detection**: Regularly audit for "God Objects," Magic Numbers, and code duplication to maintain high maintainability[cite: 32, 33, 34, 145].
